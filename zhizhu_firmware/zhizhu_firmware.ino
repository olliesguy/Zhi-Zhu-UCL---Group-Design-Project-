/*
 * Modelled around the Kritzelbot programme
 */
...
// distance between both motors (axis) 78 cm
#define AXIS_DISTANCE_X 780
#define AXIS_DISTANCE_Y 780
...
float m2s;
Servo servo;
Encoder encX(ENCX1, ENCX2);
Encoder encY(ENCY1, ENCY2);
...
void loop() {
  ...
  byte newSwitch = digitalRead(LIFT);
  if (newSwitch != switchPos) {
    if (newSwitch) {
      servo.write(PEN_UP_POS);
    }   
    else  {
  ...
  long newPosX = encX.read();
  long newPosY = encY.read();
  // posX += newPosX;
  // posY += newPosY;
